...
public class LivreClientController implements Initializable {
    ...
    {
        List <Livre> AllProducts  = new ArrayList();
        
            for (Livre p:ls.listerLivres(x))
        {
            AllProducts.add(p);
        }
        ...
        List <Livre> ThreeProducts= new ArrayList();
        for (Livre p:AllProducts )
        {
            if(i==0)
            {
                ThreeProducts.add(p);
      ...
      public Pane AddPane( List<Livre> ThreeProduct)
    {
        Pane pane = new Pane();
        pane.setStyle(" -fx-background-color: white");
                    int k =1;
                    for (Livre p3:ThreeProduct )
                        {
                         if(k == 1)
                            {
                                Pane pane2=new Pane();
