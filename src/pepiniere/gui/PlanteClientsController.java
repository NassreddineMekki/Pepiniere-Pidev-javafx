 ...
 * @author ASUS
 */
public class PlanteClientsController implements Initializable {

   
    @FXML
    private JFXListView<Pane> ListView_Produits;
    PlanteServices ls = new PlanteServices() ; 
    ...
    {
        List <Plante> AllProducts  = new ArrayList();
        
            for (Plante p:ls.listerPlante(x))
        {
            AllProducts.add(p);
        }
        ...
        List <Plante> ThreeProducts= new ArrayList();
        for (Plante p:AllProducts )
        {
            if(i==0)
            {
                ThreeProducts.add(p);
      ...
      public Pane AddPane( List<Plante> ThreeProduct)
    {
        Pane pane = new Pane();
        pane.setStyle(" -fx-background-color: white");
                    int k =1;
                    for (Plante p3:ThreeProduct )
                        {
                         if(k == 1)
                            {
                                Pane pane2=new Pane();
        ...
        pane2.setStyle(" -fx-border-radius: 10;-fx-border-color: #07AF34 ;-fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.5), 8, 0, 0, 0); ");
           ...
           {ex.printStackTrace();}
                        ...
                        HBox hb=new HBox(t);
                                ...
                                hb.setStyle("-fx-background-color: #B2FF33; -fx-background-radius: 0 0 0 10;");
