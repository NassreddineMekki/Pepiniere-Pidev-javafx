 ...
 * @author ASUS
 */
public class SoinsClientController implements Initializable {

 
  @FXML
    private JFXListView<Pane> ListView_Produits;
    SoinServices ls = new SoinServices(); 
    ...
    {
        List <Soin> AllProducts  = new ArrayList();
        
            for (Soin p:ls.listerSoin(x) )
        {
            AllProducts.add(p);
        }
        ...
        List <Soin> ThreeProducts= new ArrayList();
        for (Soin p:AllProducts )
        {
            if(i==0)
            {
                ThreeProducts.add(p);
      ...
      public Pane AddPane( List<Soin> ThreeProduct)
    {
        Pane pane = new Pane();
        pane.setStyle(" -fx-background-color: white");
                    int k =1;
                    for (Soin p3:ThreeProduct )
                        {
                         if(k == 1)
                            {
                                Pane pane2=new Pane();
